...
class DiscreteVAE(nn.Module):
    def __init__(
        ...
        enc_layers.append(nn.Conv2d(enc_chans[-1], enc_chans[-1], 4, 2))
        enc_layers.append(nn.MaxPool2d(kernel_size=2, stride=1))

        dec_layers.insert(1, nn.ConvTranspose2d(256, 256, 4, 2))
        dec_layers.insert(2, nn.Upsample(size=(25, 25), mode='nearest'))
        ...
        # dec_layers.append(nn.Conv2d(channels, channels, 3, 1))
        # dec_layers.append(nn.Upsample(size=(100, 100), mode='bilinear', align_corners=False))

        # dec_layers.insert(0, nn.ConvTranspose2d(512, 512, 4, 1))
        # dec_layers.insert(1, nn.Conv2d(256, 256, 3, 1))
    ...
    def forward(
        ...
        # logits = logits.view(-1,8192*12*12)
        # logits = self.fc(logits)
        # logits = logits.view(-1, 8192, 10, 10)
        # logits = self.additional_layers(logits)
